...
//        菜单栏中，添加菜单的（一般不添加菜单项
        ...
        madeWuqiItem.setAccelerator(KeyStroke.getKeyStroke(KeyEvent.VK_R, ActionEvent.CTRL_MASK));//设置组合键 Ctrl+R
//        madeWuqiItem.addActionListener(new ActionListener());//接口是不能new实例化的。
        ...
        JMenu kerenMenu = new JMenu("客人来了");
        JMenuItem sjcMenuItem = new JMenuItem("随机抽取区间");
        sjcMenuItem.addActionListener(jLabelActionListener);
        kerenMenu.add(sjcMenuItem);
        bar.add(kerenMenu);
    ...
    public void initJLabel(){//初始化标签组件
        ...
        getContentPane().add(new JButton("A"));//将标签加入到窗体的顶层容器中，让它可以显示 
        getContentPane().add(new JButton("B"));//将标签加入到窗体的顶层容器中，让它可以显示 
    }
