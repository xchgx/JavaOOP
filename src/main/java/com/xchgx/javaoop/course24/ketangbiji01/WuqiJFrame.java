...
public class WuqiJFrame extends JFrame{
    ...
    public WuqiJFrame() throws HeadlessException {//构造器
        ...
        manager.madeNwuqi(30);//产生n把武器-王之财宝
    }
    private  void initWindow(){
        ...
        btn.addActionListener(new ActionListener() {//匿名内部（实现）类
            ...
            public void actionPerformed(ActionEvent e) { //单击事件触发的方法
                ...
                int[] wuqis = manager.randomExtract(4);//随机抽区间
                int[] jss = manager.findJinshanshan(wuqis);//搜索可以召唤出金闪闪的三把武器
                panel.removeAll();//把容器中的所有组件都删除。
                JPanel p = new JPanel();
                for(int i=0; i<wuqis.length; i++){//遍历区间武器
                    int wq = wuqis[i];
                    JButton b = new JButton("武器"+wq+"号");
                    //这个按钮是我们找到的三把武器中的一把么？
                    //11,13,15, 现在10,不属于召唤武器，就忽视，不特殊处理
                    //现在区间的武器是13了，属于召唤武器，就特殊处理-按钮粉色背景
                    boolean belong = manager.belongToFinded(jss, wq); //是否为召唤武器
                    if(belong){//如果belong==true，表示当前这把武器是可以召唤金闪闪的
                        b.setBackground(Color.pink);//给当前按钮添加背景色
                    }
